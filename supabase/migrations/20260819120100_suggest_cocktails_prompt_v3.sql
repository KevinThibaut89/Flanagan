-- Third version of the bartender's prompt: v2 plus the house book.
--
-- The recipe library (20260819120000_recipe_library.sql) gives the edge
-- function a list of drinks the Barkeep has served before that are close to
-- the present request. v3 adds a {{LIBRARY}} placeholder and a short section
-- telling the model how to treat that list: serve a listed drink when it fits,
-- vary it when asked, ignore it when it does not, and never let it override
-- the inventory law. Model, effort and token ceiling are unchanged.
--
-- The base text is the v2 row *as it was live* on 2026-08-19 — it had been
-- re-tuned in place after 20260818160000_prompt_v2.sql seeded it, and that
-- live text (md5 010ba04f53a8109c170dbaa804800559) is what this file carries
-- verbatim, so that the repository once again says what the database runs.
-- A copy of the live v2 row is in supabase/backups/20260819-pre-library/.
--
-- Rollback is the pair of updates in reverse (see that folder's rollback.sql).
...
{{INVENTORY}}$prompt$,
  ...
  16000,
  'low',
  'v3: adds {{LIBRARY}} — the closest recipes from library_recipes, retrieved by embedding and injected by the edge function as reference. Base text is the live v2 of 2026-08-19, otherwise unchanged; same model and settings.'
);
