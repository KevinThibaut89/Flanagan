...
/** What `library_taste_profile(p_user_id)` returns. */
...
/**
 * The `{{TASTE}}` block: the drinker's own signals, sanitised and bracketed
 * like `{{LIBRARY}}`. A null or empty profile renders as "(no taste history
 * yet)" — the prompt says what that means, and a new drinker's prompt stays
 * byte-for-byte boring.
 */
export function formatTasteBlock(profile: TasteProfile | null): string {
  const liked = (profile?.liked_tags ?? []).map((tag) => oneLine(tag, 20)).filter(Boolean);
  const disliked = (profile?.disliked_tags ?? []).map((tag) => oneLine(tag, 20)).filter(Boolean);
  const avoid = (profile?.avoid_titles ?? []).map((title) => oneLine(title, 60)).filter(Boolean);

  if (liked.length === 0 && disliked.length === 0 && avoid.length === 0) {
    return 'TASTE:\n(no taste history yet)\nEND TASTE';
  }

  const lines = [
    liked.length ? `They tend to like: ${liked.join(', ')}` : null,
    disliked.length ? `They tend to avoid: ${disliked.join(', ')}` : null,
    avoid.length ? `Do not suggest (they said no): ${avoid.join('; ')}` : null,
...
export function libraryRowToDraft(row: LibraryRow, query: string) {
  return {
    // The stable identity a thumb attaches to. Fresh model answers get theirs
    // from library_upsert; here the row simply is the library.
